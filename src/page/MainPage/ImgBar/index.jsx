	...
	return (
		<div className={style.imgBar}>
			<Input
				...
				onPressEnter={() => search()}
			/>
			<Button shape="circle" icon={<SearchOutlined className={style.btn} />} className={style.btnspace2} onClick={() => search()} />
			<Button
				shape="circle"
			...
			/>
			<div className={style.imgBlock}>{imgCard(imgs)}</div>
		</div>
	);
};
...
const imgCard = (imgs) => {
	...
	return imgs.map((item, index) => {
		return <img alt="" src={'data:image/png;base64,' + item.imgData} key={item.imgId} className={style.imgCard} onClick={handleClick} />;
	});
};
